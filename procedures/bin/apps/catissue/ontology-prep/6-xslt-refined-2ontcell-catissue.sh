...
INPUT_DIR=$I2B2_ADMIN_PROCS_WORKSPACE/$JOB_NAME/$REFINED_METADATA_CATISSUE_DIRECTORY
#
# And it must exist!
if [ ! -d $INPUT_DIR ]
then
...
OUTPUT_DIR=$I2B2_ADMIN_PROCS_WORKSPACE/$JOB_NAME/$METADATA_SQL_CATISSUE_DIRECTORY/$DB_TYPE/ontcell
#
...
STYLESHEET=$I2B2_ADMIN_PROCS_HOME/xslt/$DB_TYPE/ont_cell_catissue_${DB_TYPE}.xsl
...
$JAVA_HOME/bin/java \
        -cp $(for i in $I2B2_ADMIN_PROCS_HOME/lib/*.jar ; do echo -n $i: ; done). \
         ...
         -xsl:$STYLESHEET \
         -s:$INPUT_DIR/$MAIN_REFINED_METADATA_CATISSUE_FILE_NAME \
