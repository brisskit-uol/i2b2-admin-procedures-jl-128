...
#   (2) Settings within the directory I2B2_ADMIN_PROCS_HOME/config/config.properties
...
INPUT_DIR=$I2B2_ADMIN_PROCS_WORKSPACE/$JOB_NAME/$PDO_DIRECTORY
#
# And it must exist!
if [ ! -d $INPUT_DIR ]
then
...
do
	...
	then
		...
		$JAVA_HOME/bin/java \
         -Dlog4j.configuration=file://$I2B2_ADMIN_PROCS_HOME/config/log4j.properties \
         -Djava.endorsed.dirs=$I2B2_ADMIN_PROCS_HOME/endorsed-lib \
         -cp $(for i in $I2B2_ADMIN_PROCS_HOME/lib/*.jar ; do echo -n $i: ; done). \
         org.brisskit.i2b2.dataimport.ImportPdo \
         -config=$I2B2_ADMIN_PROCS_HOME/config/config.properties \
         -import=$f \
		 -append=false \
         >>$LOG_FILE 2>>$LOG_FILE
